...
def main() -> None:
    """Generate all diagnostic figures for technical reporting."""
    ...
    for idx, (machine, group) in enumerate(telemetry.groupby("Machine ID")):
        plt.plot(
            ...
            [idx] * len(group),
            "|",
            label=f"{machine} Telemetry",
            markersize=6,
            alpha=0.6,
        )

    colors = {
        "Unplanned Failure": "red",
        "False Alarm": "orange",
        "Scheduled Maintenance": "green",
    }
    ...
    plt.title(
        "Fleet Operating Telemetry & Ground-Truth Event Timeline (6 Months)",
        fontsize=13,
        fontweight="bold",
        pad=12,
    )
    ...
    plt.plot(
        dz127["Timestamp"],
        dz127["Hyd Oil Temp Max"],
        color="crimson",
        label="Hydraulic Oil Temp Max (°C)",
        linewidth=1.5,
    )
    plt.axvline(
        pd.Timestamp("2025-04-18 17:00:00"),
        color="black",
        linestyle=":",
        linewidth=2.2,
        label="Unplanned Failure (April 18)",
    )
    plt.axhline(
        85,
        color="orange",
        linestyle="--",
        label="OEM Warning Threshold (85°C)",
        linewidth=1.2,
    )
    plt.axhline(
        95,
        color="red",
        linestyle="--",
        label="OEM Critical Threshold (95°C)",
        linewidth=1.2,
    )
    plt.title(
        "DZ-127 Hydraulic System Thermal Runaway Leading to Failure",
        fontsize=13,
        fontweight="bold",
        pad=12,
    )
    ...
    l1 = ax1.plot(
        ...
        dz118["Oil Temp Max"],
        color="tab:red",
        label="Engine Oil Temp Max (°C)",
        linewidth=1.5,
    )
    l2 = ax2.plot(
        dz118["Timestamp"],
        dz118["Blow-by Press Max"],
        color="tab:blue",
        linestyle="--",
        label="Blow-by Pressure Max (kPa)",
        linewidth=1.5,
    )
    l3 = ax1.axvline(
        ...
        linestyle=":",
        linewidth=2.2,
        label="Unplanned Failure (May 23)",
    )
    ax1.set_title(
        "DZ-118 Engine Thermal & Blow-by Degradation Pre-Failure",
        fontsize=13,
        fontweight="bold",
        pad=12,
    )
    ...
    ax_a.plot(
        dz142["Timestamp"],
        dz142["Coolant Temp Max"],
        color="darkorange",
        linewidth=1.3,
    )
    ax_a.axvline(
        pd.Timestamp("2025-03-22 05:00:00"),
        color="orange",
        linestyle=":",
        linewidth=2,
        label="False Alarm (1-hr Spike)",
    )
    ax_a.set_title(
        "DZ-142: False Alarm (Isolated Spike)", fontsize=12, fontweight="bold"
    )
    ...
    ax_b.axvline(
        ...
        linestyle=":",
        linewidth=2,
        label="True Failure (Sustained Rise)",
    )
    ax_b.set_title(
        "DZ-118: True Breakdown (Multi-Day Trend)", fontsize=12, fontweight="bold"
    )
